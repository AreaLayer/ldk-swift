...
extension Bindings {
	...
	public class Stfu: NativeTypeWrapper {
		...
		public func setChannelId(val: ChannelId) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKStfu>) in
					Stfu_set_channel_id(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public init(channelIdArg: ChannelId, initiatorArg: UInt8) {
			// native call variable prep


			// native method call
			let nativeCallResult = Stfu_new(channelIdArg.dynamicallyDangledClone().cType!, initiatorArg)
