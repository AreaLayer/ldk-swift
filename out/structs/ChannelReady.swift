...
extension Bindings {
	...
	public class ChannelReady: NativeTypeWrapper {
		...
		public func setChannelId(val: ChannelId) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKChannelReady>) in
					ChannelReady_set_channel_id(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public init(channelIdArg: ChannelId, nextPerCommitmentPointArg: [UInt8], shortChannelIdAliasArg: UInt64?) {
			// native call variable prep
			...
			let nativeCallResult = ChannelReady_new(
				channelIdArg.dynamicallyDangledClone().cType!, nextPerCommitmentPointArgPrimitiveWrapper.cType!,
