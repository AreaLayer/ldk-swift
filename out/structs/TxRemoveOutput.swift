...
extension Bindings {
	...
	public class TxRemoveOutput: NativeTypeWrapper {
		...
		public func setChannelId(val: ChannelId) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					...
					TxRemoveOutput_set_channel_id(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public init(channelIdArg: ChannelId, serialIdArg: UInt64) {
			// native call variable prep


			// native method call
			let nativeCallResult = TxRemoveOutput_new(channelIdArg.dynamicallyDangledClone().cType!, serialIdArg)
