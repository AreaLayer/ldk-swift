...
extension Bindings {
	...
	public class TxRemoveInput: NativeTypeWrapper {
		...
		public func setChannelId(val: ChannelId) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKTxRemoveInput>) in
					TxRemoveInput_set_channel_id(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public init(channelIdArg: ChannelId, serialIdArg: UInt64) {
			...
			// native method call
			let nativeCallResult = TxRemoveInput_new(channelIdArg.dynamicallyDangledClone().cType!, serialIdArg)
