...
extension Bindings {
	...
	public class Shutdown: NativeTypeWrapper {
		...
		public func setChannelId(val: ChannelId) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKShutdown>) in
					Shutdown_set_channel_id(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public init(channelIdArg: ChannelId, scriptpubkeyArg: [UInt8]) {
			// native call variable prep
			...
			// native method call
			let nativeCallResult = Shutdown_new(
				channelIdArg.dynamicallyDangledClone().cType!, scriptpubkeyArgVector.cType!)
